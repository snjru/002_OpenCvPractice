...
int main() {
    ...
    // --- K-Means Clustering for Dice Pip Grouping ---
    // 1. Prepare data for kmeans (CV_32F matrix)
    Mat img_kmeans_result = img_original.clone();
    int pipCount = allEyes.size();
    if (pipCount >= 3) { // Need at least as many pips as dice
        Mat data(pipCount, 2, CV_32F);
        for (int i = 0; i < pipCount; i++) {
            data.at<float>(i, 0) = allEyes[i].center.x;
            data.at<float>(i, 1) = allEyes[i].center.y;
        }
        ...
        // 4. Define colors for each dice group (e.g., Red, Yellow, Blue)
        Scalar clusterColors[] = {
            Scalar(0, 0, 255),   // Dice 0: Red
            Scalar(0, 255, 255), // Dice 1: Yellow
            Scalar(255, 0, 0)    // Dice 2: Blue
        };

    
        // 5. Draw grouped pips and dice centers
        for (int i = 0; i < pipCount; i++) {
            int clusterIdx = labels.at<int>(i);
            // Draw pip center with group color
            circle(img_kmeans_result, allEyes[i].center, 8, clusterColors[clusterIdx], -1); 
        }

        for (int i = 0; i < K; i++) {
            Point2f diceCenter(centers.at<float>(i, 0), centers.at<float>(i, 1));
            // Draw calculated Dice Center (kmeans center)
            circle(img_kmeans_result, diceCenter, 20, clusterColors[i], 3); 
            drawMarker(img_kmeans_result, diceCenter, clusterColors[i], MARKER_CROSS, 40, 3);
            
            // Put Dice ID or Value
            int value = countNonZero(labels == i); // Get number of pips in this cluster
            putText(img_kmeans_result, "Value: " + to_string(value), 
                    diceCenter + Point2f(30, -30), FONT_HERSHEY_SIMPLEX, 3, clusterColors[i], 3);
        }
    }
    ...
    string win3 = "After kmeans Image";
    int width = 600;
    int height = 450;
